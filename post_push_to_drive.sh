...
LOCK_FILE="$STATE_DIR/post_push.lock"
DRY_RUN=false
VERBOSE=false
ASSUME_YES=false
MIRROR=false
...
usage() {
  cat <<'EOF'
Uso: post_push_to_drive.sh [--dry-run] [--verbose] [--yes] [--mirror]
  --dry-run   Ejecuta rclone en modo simulacion
  --verbose   Aumenta el nivel de log
  --yes       No pedir confirmacion
  --mirror    Usa rclone sync (refleja borrados en Drive)
EOF
}
...
LOG_FILE="$LOG_DIR/post_push_$(date +%Y%m%d_%H%M%S).log"
